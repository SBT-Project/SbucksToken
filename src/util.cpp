...
int nAnonymizeSbucksTokenAmount = 5000;
...
#else
    const char* pszModule = "sbuckstoken";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SbucksToken
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SbucksToken
    // Mac: ~/Library/Application Support/SbucksToken
    // Unix: ~/.SbucksToken
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SbucksToken";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "sbuckstoken.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "sbuckstokend.pid"));
