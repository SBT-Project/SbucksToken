...
static const int MAX_OUTBOUND_CONNECTIONS = 64; // 64 outbound connections to SBT
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "SbucksToken " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SbucksToken is probably already running."), addrBind.ToString());
        else
