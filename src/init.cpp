...
void Shutdown()
{
    ...
    RenameThread("sbuckstoken-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: sbuckstoken.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: sbuckstokend.pid)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 10548 or testnet: 20548)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizesbuckstokenamount=<n> " + _("Keep N SbucksToken anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. SbucksToken is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. SbucksToken is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SbucksToken version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "SbucksToken server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of SbucksToken") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart SbucksToken to complete") << "\n";
    ...
    nAnonymizeSbucksTokenAmount = GetArg("-anonymizesbuckstokenamount", 0);
    if(nAnonymizeSbucksTokenAmount > 999999) nAnonymizeSbucksTokenAmount = 999999;
    if(nAnonymizeSbucksTokenAmount < 2) nAnonymizeSbucksTokenAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeSbucksTokenAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize SbucksToken Amount %d\n", nAnonymizeSbucksTokenAmount);
