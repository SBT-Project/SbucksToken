...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSbucksTokenAmount"))
        settings.setValue("nAnonymizeSbucksTokenAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeSbucksTokenAmount = settings.value("nAnonymizeSbucksTokenAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeSbucksTokenAmount"))
        SoftSetArg("-anonymizeSbucksTokenamount", settings.value("nAnonymizeSbucksTokenAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeSbucksTokenAmount:
            nAnonymizeSbucksTokenAmount = value.toInt();
            settings.setValue("nAnonymizeSbucksTokenAmount", nAnonymizeSbucksTokenAmount);
            emit anonymizeSbucksTokenAmountChanged(nAnonymizeSbucksTokenAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
