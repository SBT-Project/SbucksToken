...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("sbuckstoken"))
        return false;
...
{
    // Convert sbuckstoken:// to sbuckstoken:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sbuckstoken://"))
    {
        uri.replace(0, 12, "sbuckstoken:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SbucksToken.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the ISbucksTokenLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query ISbucksTokenLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "sbuckstoken.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SbucksToken-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  sbuckstoken-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("SbucksToken-Qt"));
