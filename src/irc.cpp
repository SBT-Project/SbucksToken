...
void ThreadIRCSeed()
{
    ...
    RenameThread("sbuckstoken-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #sbuckstokencoinTEST\r");
            Send(hSocket, "WHO #sbuckstokencoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #sbuckstokencoin\n");
            // randomly join #sbuckstokencoin00-#sbuckstokencoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #sbuckstokencoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #sbuckstokencoin%02d\r", channel_number).c_str());
        };
